...
# Gerando uma versão para publicação

Para desenvolvimento, após realizar alterações no projeto, execute:

```text
GERAR_POSTAR.bat
```

O processo recompila o TauScreen usando a versão atual do código e gera o pacote destinado à distribuição.

A versão pública deve ser gerada a partir da configuração:
...
Arquivos exclusivos do ambiente de desenvolvimento não devem ser incluídos no pacote destinado ao usuário, como:
...
Utilize o processo de geração do projeto para produzir a versão de publicação.
